...
pub(crate) fn actions(app: &mut App, ui: &mut egui::Ui) {
    ...
    if ui::dialog_button(ui, "Save", named).on_hover_text("Keep this rule for any other project").clicked() {
    ...
    ui.add(egui::TextEdit::singleline(&mut app.pattern_tool_name).desired_width(field).hint_text("Name this rule"));
}
