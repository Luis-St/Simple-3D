...
pub(crate) fn stages(app: &mut App, ui: &mut egui::Ui, id: NodeId) {
    ...
    for index in 0..used {
        ...
        ui.horizontal(|ui| {
            ...
            if index + 1 == used && used > 1 {
                ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                    let cross = ui
                        .add_sized(egui::Vec2::splat(DROP_STAGE), egui::Button::new("\u{00d7}"))
                    ...
                    // ask the context where it was drawn. It senses nothing;
                    // the button above answers the pointer.
        ...
        for key in pattern::stage_keys(index) {
            ...
            // Drawn without the stage number the label carries. The label is
            // what a value field answers to across a relayout, so four stages
            // cannot share the word "Copies" -- but the stage they sit under
            // has just been named, and "1 Copies" under "Stage 1" says it
            // twice and reads as neither English nor a number.
...
pub(crate) fn describe(stage: &pattern::Stage, unit: simple3d_core::unit::Unit) -> String {
    ...
    if stage.mode == StageMode::Turn {
        if stage.turn.abs() > 1e-9 {
            what.push(format!("turning {} deg about {axis}", simple3d_core::unit::format_number(stage.turn, 1)));
        }
        if stage.radius.abs() > 1e-9 || stage.growth.abs() > 1e-9 {
            what.push(format!(
                "at radius {} {}",
                simple3d_core::unit::format_length(stage.radius, unit),
                unit.suffix()
            ));
        }
        if stage.rise.abs() > 1e-9 {
            what.push(format!("rising {} {}", simple3d_core::unit::format_length(stage.rise, unit), unit.suffix()));
        }
    } else {
