...
impl Frame {
    ...
    /// that has to be seen through the model rather than hidden by it -- is
    /// written with `depth_test` false, and never writes depth of its own.
    ...
    /// A line drawn over whatever is already there, depth ignored: the second,
    /// faint pass of the origin axes, which has to show through a solid sitting
    /// on the origin.
