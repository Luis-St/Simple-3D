...
/// Where one axis is drawn: the point its two arms leave from, how long each
/// arm is, and the distance its fade is measured against. Shared by both
/// engines, so the GPU's axes stand exactly where the CPU's do.
